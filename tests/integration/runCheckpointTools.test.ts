...
describe('deals run tools over the live MCP surface', () => {
  it('registers deals.run_checkpoint and deals.run_resume with the dashboard tools', async () => {
    ...
    expect(names).toContain('deals.run_checkpoint');
    expect(names).toContain('deals.run_resume');
    // Backward compatibility: Phase 1–3 tools keep their names.
    expect(names).toContain('dashboard.upsert');
    expect(names).toContain('browser.extract_many');
  });
  ...
  it('is absent when the deployment configures no dashboards', async () => {
    ...
    const names = (response.body.result?.tools as { name: string }[]).map((tool) => tool.name);
    expect(names).not.toContain('deals.run_checkpoint');
    expect(names).not.toContain('dashboard.upsert');
    expect(names).toContain('browser.extract_many');
  });
  ...
  it('a run that stops mid-traversal resumes from its checkpoint instead of re-searching', async () => {
    ...
    const first = await client.callTool('deals.run_checkpoint', {
    ...
    const resumed = (
      await client.callTool('deals.run_resume', {})
    ...
    const second = (
      await client.callTool('deals.run_checkpoint', {
        runId: 'deals-2026-08-29',
    ...
    const byName = (
      await client.callTool('deals.run_resume', { runId: 'deals-2026-08-29' })
    ).body.result?.structuredContent as unknown as ResumeResult;
  ...
  it('reads take dashboards:read; writes require deals:write', async () => {
    ...
    const resume = await reader.callTool('deals.run_resume', {});
    expect(resume.body.result?.isError).not.toBe(true);

    const write = await reader.callTool('deals.run_checkpoint', { runId: 'r', verifiedIds: ['ebay-1'] });
  ...
  it('any dashboard write scope satisfies a read; a browser-only token satisfies neither', async () => {
    ...
    const vacation = await clientWith(await mintToken('vacation:write')).callTool('deals.run_resume', {});
  ...
  it('one caller cannot resume another caller\'s run', async () => {
    ...
    await mine.callTool('deals.run_checkpoint', { runId: 'owned-by-a', verifiedIds: ['ebay-1'] });

    const byName = (
      await theirs.callTool('deals.run_resume', { runId: 'owned-by-a' })
    ).body.result?.structuredContent as unknown as ResumeResult;
    expect(byName.found).toBe(false);

    const write = await theirs.callTool('deals.run_checkpoint', { runId: 'owned-by-a', verifiedIds: ['ebay-9'] });
    expect(write.body.result?.isError).toBe(true);
    expect(errorOf(write).code).toBe('ACTION_BLOCKED');

    const owner = (
      await mine.callTool('deals.run_resume', { runId: 'owned-by-a' })
    ).body.result?.structuredContent as unknown as ResumeResult;
  ...
  it('refuses a checkpoint that tries to park page content in it', async () => {
    const client = clientWith(await mintToken('deals:write'));
    const oversized = await client.callTool('deals.run_checkpoint', {
      runId: 'blob-attempt',
      notes: 'x'.repeat(5000),
    });
    expect(oversized.body.result?.isError).toBe(true);

    const unknownField = await client.callTool('deals.run_checkpoint', {
      runId: 'blob-attempt',

...
export const TOOL_CATALOG: readonly ToolCatalogEntry[] = [
  ...
  },
  {
    ...
    timeoutMs: DEFAULT_TIMEOUT_MS,
    description:
      'Navigate a tab to an allowed HTTPS URL. Returns final URL, title, origin, navigation status, the new page revision, and blockedSubresources: every origin the local site allowlist refused an image, script, frame or fetch from while the page loaded (origin, code, request count, one example URL), so a module that renders as an empty skeleton because its data host is outside the allowlist can be reported by host instead of guessed; an empty list on a page that still renders nothing means the gap is not a policy block. browser_wait returns the same tally read again later. A redirect the SITE makes into a URL local policy refuses (an http:// or off-allowlist error interstitial) is never loaded, but the requested URL is retried once: a retry that lands reports the refused hop in blockedSubresources; a retry redirected there again fails REDIRECT_BLOCKED (retryable, details.blockedUrl) with the tab parked on about:blank. A redirect into a sign-in or transaction path stays ACTION_BLOCKED and is not retried.',

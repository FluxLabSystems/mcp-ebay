...
Six profiles exist today: `ebay.ca.v1`, `kijiji.ca.v1`, `zazzle.com.v1`,
`wardrobe-vendors.v1`, `office-sources.v1` (shipped 2026-09-05, below) and
`jobs-sources.v1` (shipped 2026-09-13, below). Everything else the five Fluxology routines read, they
...
### `jobs-sources.v1` — policy-only — SHIPPED 2026-09-13

| | |
| --- | --- |
| Routine | `fluxology-jobs-run` |
| Hosts | `packages/site-jobs/src/sources.ts` — 10 boards (`jobbank.gc.ca`, `indeed.com` (covers `ca.indeed.com` and the `to.indeed.com` short links), `talent.com`, `eluta.ca`, `jobillico.com`, `simplyhired.ca`, `glassdoor.ca`, `workopolis.com`, `linkedin.com`, `ziprecruiter.com`), 3 aggregators (`adzuna.ca`, `jooble.org`, `careerbeacon.com`), 8 union / apprenticeship / public channels (`ualocal46.org`, `smwia-l30.com`, `iw721.org`, `liunalocal183.ca`, `skilledtradesontario.ca`, `apprenticesearch.com`, `ontario.ca`, `georgebrown.ca`) and 11 employer ATS hosts (`myworkdayjobs.com`, `smartrecruiters.com`, `greenhouse.io`, `lever.co`, `bamboohr.com`, `icims.com`, `jobvite.com`, `applytojob.com`, `njoyn.com`, `recruiting.ultipro.com` and `workforcenow.adp.com` — the two recruiting hosts only, never the payroll apexes). `kijiji.ca` stays on `kijiji.ca.v1`; `craigslist.org` and `monster.ca` (a redirect to monster.com's résumé product on 2026-09-13) are not on it. |
| Evidence | The original row: `fluxology-jobs-run__2026-09-04T23-18-41` (Job Bank `&page=N` paging confirmed through WebFetch; the Indeed single-token degradation) and `fluxology-jobs-run__2026-09-07T22-16-51` (`indeed+coverage_gap+ca-indeed-viewjob-401-blocks-canonical-url-verification`: the canonical `viewjob` page answers HTTP 401 to WebFetch, so the connector was unfalsifiable against its own source). The `ORIGIN_DENIED` the row waited on was never spent: on 2026-09-13 the operator directed, in the live CI session, that the jobs dashboard's default sources and the connectors it uses be expanded, and ratified the roster in-session — the same precedent as the office roster of 2026-09-05 (fingerprint `operator+skill_gap+jobs-default-sources-and-connectors-expansion-2026-09-13`; the `source` field on every entry records it). Every host was checked to resolve on 2026-09-13 (plain-fetch status recorded per entry); none was read through the Bridge yet. |
| Why the Bridge | Indeed, SimplyHired, Glassdoor, Workopolis, Adzuna, Jooble, CareerBeacon and George Brown answer 401/403 to plain fetch; the ATS hosts render client-side; `iw721.org` has been unreadable by WebFetch since 2026-09-05 and its intake state has never been read. A snapshot answers "empty lane or blocked lane" in one call, and reads the canonical posting the connector's figure is checked against. Job Bank, talent.com, Eluta and the readable channels are on the roster so the lane is one profile. |
| Walls | `createResearchProfile` defaults plus the job-board specifics: by accessible name `apply`, `apply on company site`, `apply with indeed` / `linkedin`, `start application`, `save job`, `create job alert`, `email me jobs`, `upload resume` / `cv`, `message recruiter`, `contact employer`, `follow company`, …; by path the `resume` / `cv` / `profile` / `saved-jobs` / `job-alerts` / `easyapply` / `candidate` / `my-account` / `onboarding` / `talent-community` segments on top of the generic `apply` / `application` / `message` / `subscribe` set. Read and navigate only: no application, no upload, no alert, no message to any employer, recruiter or union — an application is the operator's own act. `browser_extract` answers `NO_EXTRACTOR_FOR_HOST` on every roster host with a reading hint that keeps required apart from preferred. The test pins that a Job Bank posting URL carrying `?applyonline=false` is **not** an endpoint (the row's own requirement), and that `applytojob.com` the host is allowed while its `/apply` path is not. |
| Still needed | Live verification per host, recorded on the entry's `needsLiveVerification`: whether the 403s and the ATS client rendering pass in a normal browser context; that LinkedIn's authwall is refused rather than satisfied; that every apply / save / alert control is refused by accessible name where its `href` is `#`. The `siteProfile` enums in `packages/protocol/src/tools.ts` are unchanged (policy-only lanes dispatch by host), so the lane needs the Windows agent rebuild only — no gateway redeploy, no connector reconnect. |
| Live so far (2026-09-14) | The lane is deployed and serving (`confirmed-live:operator+skill_gap+jobs-default-sources-and-connectors-expansion-2026-09-13`, jobs fire 22:2xZ), recorded per entry in `verifiedLive`. **ca.indeed.com** renders the canonical `viewjob` page in full; a dead vacancy redirects off-host to `www.indeed.com`, which the roster deliberately omits, so the refusal is the dead-vs-live test (`www.indeed.com` must stay off the roster for that reason). **iw721.org** renders with no wall and its intake state was read for the first time — by `browser_screenshot`, because the page is running body text and `browser_snapshot` returns only headings and links (a method rule now in the jobs skill, not a Bridge defect). **jobillico.com**: the apex renders, both constructed keyword-search URL shapes 404, the footer exposes only a path-segment family, and the on-page search form fills but does not navigate — path-browse only until the shape is recovered (`gateway+coverage_gap+jobillico-documented-search-url-shapes-404-and-the-on-page-search-form-does-not-navigate`). **apprenticesearch.com** is registration-walled — HTTP 200 marketing pages, zero signed-out inventory — so a read-only profile reaches none of its value; it stays on the roster (ratified) but earns no Bridge read (`gateway+coverage_gap+apprenticesearch-job-board-is-behind-a-registration-wall`). **liunalocal183.ca** is the parent union site only: `/training/` 404s and the LiUNA Local 183 Training Centre lives on `183training.com`, which WebFetch reads (HTTP 200) and which is on **no roster** — under rule 1 above it joins only on a filed `ORIGIN_DENIED` or the operator's ratification, and today no Bridge read of it is owed (`skills/fluxology-jobs-run+skill_gap+liuna-183-roster-entry-names-a-host-whose-training-pages-404-the-training-centre-is-a-separate-host`). |
| Risk | Every host here has an apply flow, and the ATS hosts are nothing but apply flows around a posting page. The walls above are the answer; a live read that reaches an application form through any control the walls did not name is filed as `gateway+connector_defect` with the accessible name and the URL, and the pattern is added. |
...
| Hosts | `packages/site-office/src/sources.ts` — 19 provider domains (`regus.com`, `spacesworks.com`, `hq.com`, `industriousoffice.com`, `wework.com`, `iqoffices.com`, `venturex.com`, `intelligentoffice.com`, `telsec.net`, `workhaus.ca`, `workplaceone.com`, `zemlar.ca` + `zemlar.com` (the `.ca` serves the site; the `.com` is a dead apex — "Index of /", product paths 404, observed 2026-09-09 — kept only so a redirect from it is not refused), `oneplan.ca`, `collabhive.ca`, `gtexecutivecentre.com`, `studio.staples.ca` (the studio subdomain only, never the retailer apex), `workplacek.com`, `office146.com`, `thefuelingstation.com`) and 8 listing/MLS/classifieds surfaces (`realtor.ca`, `liquidspace.com`, `office-hub.com`, `spacelist.ca`, `coworkingcafe.com`, `commercialcafe.com`, `loopnet.ca`, and `craigslist.org` since 2026-09-14 — every city host beneath it; `accounts.` and `post.` and the per-city `/reply/`, `/flag/`, `/edit/`, `/manage/`, `/post/` segments are protected endpoints, and `reply` / `flag` / `post to classifieds` / `my account` are refused by accessible name) |
| Evidence | `fluxology-office-run__2026-09-05T13-50-56` carried the live `ORIGIN_DENIED` for `www.regus.com` this row was waiting on, and the operator ratified the roster in-session on 2026-09-05 (the `source` field on every entry records it). `fluxology-office-run__2026-09-14T00-42-18` carried the live `ORIGIN_DENIED` for `www.craigslist.org` (the ONE probe the office SKILL.md's craigslist bullet (5) owed, spent on the area-search URL, none retried, kijiji.ca and regus.com read normally in the same session); craigslist.org is a source the committed SKILL.md's pathway table names, so the entry shipped under the roster rule with its own walls. Its `needsLiveVerification` records what the next fire confirms: whether post pages render headlessly, that the reply control is refused by name, and that the www area URL commits rather than 301-ing. |

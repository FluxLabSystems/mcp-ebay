...
| Still needed | Live verification per host, recorded on the entry's `needsLiveVerification`: whether Indeed's 401, the 403s and the ATS client rendering pass in a normal browser context; that LinkedIn's authwall is refused rather than satisfied; that every apply / save / alert control is refused by accessible name where its `href` is `#`. The `siteProfile` enums in `packages/protocol/src/tools.ts` are unchanged (policy-only lanes dispatch by host), so the lane needs the Windows agent rebuild only — no gateway redeploy, no connector reconnect. |

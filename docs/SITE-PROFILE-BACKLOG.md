...
| Evidence | `fluxology-jobs-run__2026-09-04T23-18-41` (Job Bank `&page=N` paging confirmed through WebFetch); the same fire's Indeed single-token degradation report. `fluxology-jobs-run__2026-09-07T22-16-51` (fingerprint `indeed+coverage_gap+ca-indeed-viewjob-401-blocks-canonical-url-verification`): the canonical `https://ca.indeed.com/viewjob?jk=a18e85115521a680` answers **HTTP 401** to WebFetch with no body, so the canonical URL the routine is told to store cannot be confirmed to resolve and a connector pay figure cannot be checked against the posting when a mirror disagrees ($22–27/h from the connector against $20–24/h in a search-result render of the same page). Not an `ORIGIN_DENIED`: that fire made no Bridge attempt, and says so. |
| Why the Bridge | Indeed degrades to a single result token under WebFetch, and the routine currently cannot tell a genuinely empty lane from a blocked one. A snapshot answers that in one call. The 2026-09-07 401 adds the second reason: the only pathway that reads a `ca.indeed.com` job page today is the connector itself, which makes the connector unfalsifiable against its own source. Job Bank paging works under WebFetch and does **not** need the Bridge — it is on the roster so the lane is one profile, not so the walk moves. |
| Still needed | An `ORIGIN_DENIED` filed against a specific Indeed URL from a live session. The jobs skill (`references/source-strategy.md`, 2026-09-08) now spends one `browser_open_and_extract` on a canonical `viewjob` URL it needs to read and files the refusal once under `indeed+coverage_gap+ca-indeed-viewjob-origin-denied`; that report is the evidence this row waits on. Whether Indeed's 401 is a plain user-agent filter that a profile's normal browser context passes is unknowable until that read happens. |

...
| Evidence | Named in the committed vacation SKILL.md. No `ORIGIN_DENIED` filed. |
...
| Still needed | Evidence, and a decision on the OTAs specifically. `booking.com` and `trip.com` are transactional in a way the resort sites are not; the safer first cut is the three brand sites plus `tripadvisor.com`, with the OTAs deferred until the brand lane has run. |
